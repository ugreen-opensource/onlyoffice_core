...
{	
	EndnotesMapping::EndnotesMapping (ConversionContext* ctx) : DocumentMapping(ctx, this)
	{
	}

	void EndnotesMapping::Apply( IVisitable* visited )
	{
		...
		if ( ( m_document != NULL ) && ( m_document->FIB->m_RgLw97.ccpEdn > 0 ) )
		{
			...
			m_context->_docx->EndnotesXML = m_pXmlWriter->TakeXmlString();
		}
	}
}
